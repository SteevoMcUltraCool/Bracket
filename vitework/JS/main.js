...
if (!window.localStorage.getItem("matches") || true) {
  ...
  let matches = JSON.parse(window.localStorage.getItem("matches"));
  matches.forEach((match) => {
    console.log(match);
    new Match(match.name, match.LR, match.round, match.game, match.players);
  });
}
